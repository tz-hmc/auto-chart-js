const o=e=>+e/16*1e3,c=e=>e/1e3/(1/16),e=[{keyCode:37,color:"#dbabff"},{keyCode:40,color:"#9494ff"},{keyCode:38,color:"#ff7a91"},{keyCode:39,color:"#6effbb"}];class t extends HTMLElement{constructor(){super()}connectedCallback(){this.render()}render(){this.innerHTML=`
        <style>
            #spinner {
                width: 100%;
                height: 100%;
        ...
        </div>`}finishPlayCallback(){this.dispatchEvent(new CustomEvent("chart-page-play-stop",{bubbles:!0,cancelable:!1,composed:!0}))}createLocalChart(){let e=document.createElement("app-chart");e.setValues({isLocal:!0,input:new i(this.chart,e,this.playerManager.LocalPlayer)}),this.flexContainer.appendChild(e),e.play(this.chart,this.songStartTimeMs).then(()=>{this.finishPlayCallback()})}createRemoteCharts(){var e=this.playerManager.RemotePlayers[0];let t=document.createElement("app-chart");this.flexContainer.appendChild(t),t.setValues({isLocal:!1,input:new a(this.chart,t,e)}),t.play(this.chart,this.songStartTimeMs)}start(){this.createLocalChart(),this.createRemoteCharts()}remove(e){this.flexContainer.removeChild(e.element)}}customElements.get("chart-page")||customElements.define("chart-page",r);class l extends HTMLElement{setValues({numberPlayerBroadcast:e,finishedUpload:t}){this.numberPlayerBroadcast|=e,this.finishedUpload|=t,this.rerender()}get input(){return this.shadow.querySelector("input")}get readyButton(){return this.shadow.querySelector("button#ready")}get spinner(){return this.shadow.querySelector("loading-spinner")}get showReady(){return this.finishedUpload}get showSpinner(){return this.startedUpload&&!this.finishedUpload}get readyText(){return this.readyClicked?"waiting for second player":"ready"}constructor(){super(),this.reset(),this.shadow=this.attachShadow({mode:"open"})}connectedCallback(){this.render(),this.rerender(),this.registerEventListeners()}reset(){this.numberPlayerBroadcast=0,this.startedUpload=!1,this.finishedUpload=!1,this.readyClicked=!1}render(){this.shadow.innerHTML=`
        <style>
            :host, .flex-container {
                top: 0;
                left: 0;
        ...
        </div>`}}customElements.get("score-page")||customElements.define("score-page",d);class p{constructor(e="",t){this.clientId=e,this.connectionManager=t,this.score=0}update({score:e}){this.score=e}}class u extends p{update({score:e,currNoteIndices:t}){super.update({score:e}),this.connectionManager.sendUpdate({score:e,currNoteIndices:t})}}class g extends p{constructor(e,t){super(e,t),this.connectionManager.setCallback({playerUpdate:this.update.bind(this)})}update({score:e,currNoteIndices:t}){super.update({score:e}),this.currNoteIndices=t}}class m{get LocalPlayer(){return this.localPlayer}get RemotePlayers(){return Object.values(this.remotePlayers)}constructor(){this.remotePlayers={},this.localPlayer=null}addLocal(e,t){this.localPlayer=new u(e,t)}addRemote(e,t){this.remotePlayers[e]=new g(e,t)}updateRemote({clientId:e,...t}){e in this.remotePlayers?this.remotePlayers[e].update(t):console.error("client id is not recognized")}removeRemote(e){delete this.remotePlayers[e]}}const y=["input-page","chart-page","score-page"];class f extends HTMLElement{pageIndex=0;connectionManager=new class{constructor(){this.playerManager=new m,this.conn=null,this.roomCode="",this.callbacks={}}isConnected(){return!!this.conn&&0<this.roomCode.length}setCallback({create:e,join:t,broadcast:s,songReady:i,roomReady:a,gameBroadcast:n}){this.callbacks={create:e,join:t,broadcast:s,songReady:i,roomReady:a,gameBroadcast:n,...this.callbacks}}connect(e="ws://localhost:9000"){this.conn=new WebSocket(e),this.conn.addEventListener("open",()=>{this.initRoom()}),this.conn.addEventListener("message",e=>{console.log(`message: ${e.data}`);e=this.receive(e.data);"room-created"===e.type?(this.setRoom(e.roomCode),this.callbacks?.create&&this.callbacks.create(e)):"room-joined"===e.type?(this.setRoom(e.roomCode),this.callbacks?.join&&this.callbacks.join(e)):"room-broadcast"===e.type?(this.setPlayers(e.peers),this.callbacks?.broadcast&&this.callbacks.broadcast(e)):"song-ready"===e.type?this.callbacks?.songReady&&this.callbacks.songReady(e):"room-ready"===e.type?this.callbacks?.roomReady&&this.callbacks.roomReady(e):"game-broadcast"===e.type&&(this.playerManager.updateRemote(e),this.callbacks?.gameBroadcast&&this.callbacks.gameBroadcast(e))})}setPlayers(e){const t=e.you,s=e.peers.filter(e=>t!==e);this.playerManager.addLocal(t,this),s.forEach(e=>{this.playerManager.addRemote(e,this)})}receive(e){return JSON.parse(e)}send(e){e=JSON.stringify(e);console.log(`sending message ${e}`),this.conn.send(e)}sendCreate(){this.send({type:"create-room"})}sendJoin(){this.send({type:"join-room",roomCode:this.roomCode})}sendReady(){this.send({type:"ready",roomCode:this.roomCode})}sendUpdate({score:e,currNoteIndices:t}){this.send({type:"game-update",score:e,currNoteIndices:t})}sendFinish(){this.send({type:"game-finish"})}initRoom(){this.roomCode=window.location.hash.split("#")[1],this.roomCode?this.sendJoin():this.sendCreate()}setRoom(e){this.roomCode=e,window.location.hash=this.roomCode}};fileManager=new class{constructor(){this.fileBuffer=null,this.chart=[]}async uploadMp3(e,t){try{await fetch(`http://localhost:3333/song/${e}`,{method:"PUT",headers:{"Content-Type":"audio/mpeg"},body:t})}catch(e){alert("Please check file format (should be mp3) and size (12MB limit).")}}async downloadMp3(e){let t=await fetch(`http://localhost:3333/songs/${e}.mp3`,{method:"GET"});this.fileBuffer=await t.arrayBuffer()}async downloadChart(e){let t=await fetch(`http://localhost:3333/charts/${e}.json`,{method:"GET"});return this.chart=await t.json(),this.chart}async playMp3(){let e=new AudioContext;const t=e.createBufferSource();t.buffer=await e.decodeAudioData(this.fileBuffer),t.connect(e.destination),t.start()}};constructor(){super(),this.reset(),this.registerEventListeners()}connectedCallback(){this.render(),this.connectionManager.setCallback({broadcast:this.broadcastCallback.bind(this),songReady:this.songReadyCallback.bind(this),roomReady:this.roomReadyCallback.bind(this)}),this.connectionManager.connect()}reset(){this.pageIndex=0,this.localChart=[]}render(){this.innerHTML=`
            <div>
                ${0===this.pageIndex?"<input-page></input-page>":""}
                ${1===this.pageIndex?"<chart-page></chart-page>":""}
                ${2===this.pageIndex?"<score-page></score-page>":""}
            </div>
        `}getRef(e){return this.querySelector(e)}broadcastCallback({}){this.getRef("input-page")?.setValues({finishedBroadcast:!0})}async songReadyCallback({}){await Promise.all([this.fileManager.downloadChart(this.connectionManager.roomCode),this.fileManager.downloadMp3(this.connectionManager.roomCode)]),this.getRef("input-page")?.setValues({finishedUpload:!0})}async roomReadyCallback({}){await this.fileManager.playMp3();var e=Date.now();this.goNextPage(),this.getRef("chart-page")?.setValues({playerManager:this.connectionManager.playerManager,chart:this.fileManager.chart,songStartTimeMs:e}),this.getRef("chart-page")?.render(),this.getRef("chart-page")?.start()}registerEventListeners(){this.addEventListener("input-page-file-upload",this.onFileUploadClicked.bind(this)),this.addEventListener("input-page-ready-click",this.onReadyClicked.bind(this)),this.addEventListener("chart-page-play-stop",this.onPlayStopped.bind(this))}goNextPage(){this.pageIndex=this.pageIndex===y.length-1?this.pageIndex:this.pageIndex+1,this.render()}onFileUploadClicked({detail:{buffer:e}}){this.fileManager.uploadMp3(this.connectionManager.roomCode,e)}onReadyClicked({}){this.connectionManager.sendReady()}onPlayStopped({}){var e=this.connectionManager.playerManager.LocalPlayer.score,t=this.connectionManager.playerManager.RemotePlayers.map(e=>e.score);this.goNextPage(),this.getRef("score-page")?.setValues({localScore:e,enemyScore:t[0]}),this.getRef("score-page")?.render(),this.connectionManager.sendFinish()}}customElements.get("page-manager")||customElements.define("page-manager",f);